...
class MY_Controller extends CI_Controller {
    ...
    $this->load->helper('error_message');
		...
		//$this->load->model('gcm/model_gcm');
		if($this->uri->segment(1) != "work_check" && $this->uri->segment(1) != "project_management") {
		...
		$this->admin_grade=$this->session->userdata('admin_grade');
		$this->site_code=$this->session->userdata('site_code');
    ...
    $response->board_category_list = $this->model_common->board_category_list();

		$this->load->view("common/inc/header");
		$this->load->view("common/inc/left",$response);
