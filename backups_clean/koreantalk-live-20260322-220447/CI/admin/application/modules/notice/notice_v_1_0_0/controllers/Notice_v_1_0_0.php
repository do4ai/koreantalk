...
class Notice_v_1_0_0 extends MY_Controller{
	...
	public function notice_list(){
  	$site_list = $this->model_common->site_list();
		...
		$this->_view(mapping('notice').'/view_notice_list',$response);
	}
    ...
    $page_num = $this->_input_check("page_num",array("ternary"=>'1'));
		...
		$result_list_count = $this->model_notice->notice_list_count($data); // 공지사항 리스트 총 카운트
    ...
    $site_list = $this->model_common->site_list();
		...
		$this->_view(mapping('notice').'/view_notice_reg', $response);
	}
    ...
    $data['img_path'] = $img_path;
		...
		$result = $this->model_notice->notice_reg_in($data); // 공지사항 등록
    ...
    $data['img_path'] = $img_path;
		...
		$result = $this->model_notice->notice_mod_up($data); // 공지사항 수정
  ...
  // 공지사항 알림 발송 
	public function notice_push_mod_up(){
		$notice_idx = $this->_input_check("notice_idx",array("empty_msg"=>"삭제 할 항목이 없습니다."));
		$notice_type = $this->_input_check("notice_type",array("empty_msg"=>"삭제 할 항목이 없습니다."));

		$data['notice_idx'] = $notice_idx;

		$check = $this->model_notice->notice_detail($data);

		$response = new stdClass();

		//알림발송(201)
		$index="101";
		$alarm_data['notice_idx'] =  $notice_idx;
		$alarm_data['title'] = "[공지사항] ".$check->title;
		$member_idx = $notice_type;
		$this->_alarm_action($member_idx,$index, $alarm_data);

		$result = $this->model_notice->notice_push_mod_up($data); // 공지사항 알림 발송 
    if($result == 0){
      $response->code = "-2";
      $response->code_msg = error_code_msg('-2'); //실패 하였습니다.
    }else{
