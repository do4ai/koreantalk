...
class Electron_book_v_1_0_0 extends MY_Controller{
	function __construct(){
		...
		$this->load->model(mapping('electron_book').'/model_product');
	}
  ...
  // 인덱스
	...
	public function product_list(){
  	$site_list = $this->model_common->site_list();
		...
		$this->_view(mapping('electron_book').'/view_product_list',$response);
	}
    ...
    $this->_list_view(mapping('electron_book').'/view_product_list_get', $response);
	}	
    ...
    $response = new stdClass();
		...
		$this->_view(mapping('electron_book').'/view_product_detail',$response);
	}
    ...
    $lecture_movie_list = $this->model_common->lecture_movie_list(); 
		...
		$this->_view(mapping('electron_book').'/view_product_reg',$response);
	}
  ...
  // 상품 등록
    ...
    $author = $this->_input_check("author",array("empty_msg"=>"저자를 입력해주세요","focus_id"=>"author"));
	  ...
	  $pdf_url = $this->_input_check("pdf_url",array("empty_msg"=>"pdf를 업로드 해주세요","focus_id"=>"pdf_url"));
		$pdf_org_name = $this->_input_check("pdf_org_name",array());
	  $lecture_movie_idx = $this->_input_check("lecture_movie_idx",array("empty_msg"=>"관련 TOPIK 영상을  선택해 주세요","focus_id"=>"lecture_movie_idx"));
    ...
    $product_contents = $this->_input_check("product_contents",array("empty_msg"=>"상품설명을 입력해주세요."));
		...
		$data['pdf_org_name'] = $pdf_org_name;
    ...
    $electron_book_idx = $this->_input_check("electron_book_idx",array());
		...
		$lecture_movie_list = $this->model_product->lecture_movie_list(); // 대 카테고리 리스트 
		$result = $this->model_product->product_detail($data); // 상품 상세
		...
		$this->_view(mapping('electron_book').'/view_product_mod',$response);
	}
