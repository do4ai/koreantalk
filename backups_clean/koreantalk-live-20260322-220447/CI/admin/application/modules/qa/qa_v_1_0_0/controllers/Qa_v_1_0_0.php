...
class Qa_v_1_0_0 extends MY_Controller{
    ...
    $qa_category = $this->_input_check("qa_category",array());
		...
		$data['s_date'] = $s_date;
		$data['e_date'] = $e_date;
    $data['qa_category'] = $qa_category;
		$data['page_no'] = ($page_num-1)*$page_size;
		$data['page_size'] = $page_size;

		$result_list = $this->model_qa->qa_list($data); // QA 리스트 가져오기 
		$result_list_count = $this->model_qa->qa_list_count($data); // QA 리스트 총 카운트
	...
	public function qa_reply_reg_in(){
		$qa_idx = $this->_input_check("qa_idx",array("empty_msg"=>"QA 코드 누락"));
		$reply_contents = $this->_input_check("reply_contents",array("empty_msg"=>"답변을 입력해주세요."));
    $member_idx = $this->_input_check("member_idx",array());

		$data['qa_idx'] = $qa_idx;
		$data['reply_contents'] = $reply_contents;

		$result = $this->model_qa->qa_reply_reg_in($data); // QA 답변 등록
