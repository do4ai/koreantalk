...
class Pg_iamport extends MY_Controller {
  ...
  public function pg_end() {
    ...
    $P_STATUS =$_REQUEST['imp_success'];
      ...
      //결제확인
        ...
        if(APP_YN =="Y"){
          ...
          $response->code = "-1"; //
          $response->code_msg = lang('lang_10391','이미 결제된 주문입니다');
          echo json_encode($response);
          exit;

        }else{
          $this->global_function->_alert(lang('lang_10392','이미 결제된 주문입니다.'),"/");
          exit;
        }
			}
		}
    ...
    if($P_STATUS=="true" ){
      ...
      if(APP_YN =="Y"){
        $response = new stdClass;
        $response->code = "-1"; //
        $response->code_msg = lang('lang_10393','결제실패 되었습니다.');
        echo json_encode($response);
        exit;
      }else{
  			$this->global_function->_alert(lang('lang_10394','결제실패 되었습니다.'),"/");
  			exit;
     }
  ...
  public function do_pg_cancel($pg_tid) {
    ...
    if($inipay->getResult('ResultCode')=="00"){
      ...
      $response->code_msg = lang('lang_10396','취소 성공');
      echo json_encode($response);
      exit;
		}else{
      $response = new stdClass;
      $response->code = "-1"; // 이미 등록된 휴대폰번호 입니다.
      $response->code_msg = lang('lang_10397','취소실패');
      echo json_encode($response);
      exit;
		}
  ...
  public function api_pg_cancel() {
    ...
    if($inipay->getResult('ResultCode')=="00"){
      $response = new stdClass;
      $response->code = "1"; // 이미 등록된 휴대폰번호 입니다.
      $response->code_msg = lang('lang_10398','취소 성공');
      echo json_encode($response);
      exit;
		}else{
      $response = new stdClass;
      $response->code = "-1"; // 이미 등록된 휴대폰번호 입니다.
      $response->code_msg = lang('lang_10399','취소실패');
      echo json_encode($response);
      exit;
		}
