...
class Faq_v_1_0_0 extends MY_Controller{
  ...
  public function faq_list(){
		$result_list = $this->model_faq->faq_list(); // FAQ 리스트 가져오기

		$response = new stdClass();

		$response->result_list = $result_list;   

		$this->_view(mapping('faq').'/view_faq_list',$response);
  }
  ...
  public function faq_list_get(){
	  ...
	  $page_size = 20000 ;
		...
		$result_list = $this->model_faq->faq_list($data); // FAQ 리스트 가져오기
		$result_list_count = $this->model_faq->faq_list_count(); // FAQ 리스트 카운트 가져오기
    ...
    $paging = $this->global_function->paging($result_list_count, $page_size, $page_num);
	  ...
	  $this->_list_view('faq/view_faq_list_get',$response);
  }
